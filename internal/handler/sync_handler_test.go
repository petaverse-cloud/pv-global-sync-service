...
func (m *mockIdx) GetPost(_ context.Context, _ int64) (*model.GlobalPostIndex, error) {
	return nil, nil
}
func (m *mockIdx) GetPostByUid(_ context.Context, _ int64) (*model.GlobalPostIndex, error) {
	return nil, nil
}
...
func (m *mockEvt) MarkProcessed(_ context.Context, ev *model.CrossRegionSyncEvent, _ string) error {
	m.m[ev.EventID] = true
	return nil
}
...
type mockTag struct {
	upsert, del, updStats bool
	tag                   *model.GlobalTagIndex
	regions               []string
}

func (m *mockTag) UpsertTag(_ context.Context, _ *model.CrossRegionSyncEvent) error {
	m.upsert = true
	return nil
}
func (m *mockTag) DeleteTag(_ context.Context, _ *model.CrossRegionSyncEvent) error {
	m.del = true
	return nil
}
func (m *mockTag) UpdateStats(_ context.Context, _, _ int64) error { m.updStats = true; return nil }
func (m *mockTag) SearchTags(_ context.Context, _ string, _ int) ([]model.GlobalTagIndex, error) {
	return nil, nil
}
func (m *mockTag) GetPopularTags(_ context.Context, _ int) ([]model.GlobalTagIndex, error) {
	return nil, nil
}
func (m *mockTag) GetTagByUID(_ context.Context, _ int64) (*model.GlobalTagIndex, error) {
	return m.tag, nil
}
func (m *mockTag) GetRegionsForTag(_ context.Context, _ int64) ([]string, error) {
...
func TestRouteEvent_PostCreated(t *testing.T) {
	...
	if err := h.routeEvent(context.Background(), makeEvent(model.EventTypePostCreated, 100, 200, "hi")); err != nil {
		t.Fatal(err)
	}
	if !f.newPost {
...
func TestRouteEvent_PostDeleted(t *testing.T) {
	...
	if !f.delPost {
		t.Error("HandleDeletedPost not called")
	}
}
func TestRouteEvent_TagCreated(t *testing.T) {
	tg := &mockTag{}
	h := &SyncHandler{tagIndexSvc: tg, log: logger.NewNop()}
	ev := makeEvent(model.EventTypeTagCreated, 0, 0, "")
	ev.Payload.TagUID = 500
	if err := h.routeEvent(context.Background(), ev); err != nil {
		t.Fatal(err)
	}
	if !tg.upsert {
		t.Error("UpsertTag not called")
	}
}
func TestRouteEvent_TagUpdated(t *testing.T) {
	tg := &mockTag{}
	h := &SyncHandler{tagIndexSvc: tg, log: logger.NewNop()}
	ev := makeEvent(model.EventTypeTagUpdated, 0, 0, "")
	ev.Payload.TagUID = 501
	if err := h.routeEvent(context.Background(), ev); err != nil {
		t.Fatal(err)
	}
	if !tg.upsert {
...
func TestRouteEvent_TagDeleted(t *testing.T) {
	tg := &mockTag{}
	h := &SyncHandler{tagIndexSvc: tg, log: logger.NewNop()}
	ev := makeEvent(model.EventTypeTagDeleted, 0, 0, "")
	ev.Payload.TagUID = 502
	if err := h.routeEvent(context.Background(), ev); err != nil {
		t.Fatal(err)
	}
	if !tg.del {
		t.Error("DeleteTag not called")
	}
}
func TestRouteEvent_TagStatsUpdated(t *testing.T) {
	...
	ev.Payload.TagUID = 503
	pc := int64(42)
	ev.Payload.TagPostCount = &pc
	if err := h.routeEvent(context.Background(), ev); err != nil {
		t.Fatal(err)
	}
	if !tg.updStats {
		t.Error("UpdateStats not called")
	}
}
...
func TestHandleSync_MissingFields(t *testing.T) {
	...
	for _, tt := range []struct{ n, b string }{{"no eventId", `{"eventType":"X","payload":{"postUid":1}}`}, {"no eventType", `{"eventUid":"e1","payload":{"postUid":1}}`}, {"empty", `{}`}} {
		...
		r.ServeHTTP(rec, req)
		if rec.Code != 400 {
			t.Errorf("%s: status=%d", tt.n, rec.Code)
		}
	}
}
...
func TestHandleGetPost_InvalidUid(t *testing.T) {
	...
	for _, uid := range []string{"abc", "-5", "12a34"} {
		...
		if rec.Code != 400 {
			t.Errorf("uid=%q status=%d", uid, rec.Code)
		}
	}
}
...
func TestHandleGetPostByUid_Found(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	r.ServeHTTP(rec, httptest.NewRequest("GET", "/index/posts/uid/888", nil))
	if rec.Code != 200 {
		t.Errorf("status=%d", rec.Code)
	}
}
func TestHandleGetPostByUid_InvalidUid(t *testing.T) {
	...
	r.Get("/index/posts/uid/{uid}", h.HandleGetPostByUid)
	rec := httptest.NewRecorder()
	r.ServeHTTP(rec, httptest.NewRequest("GET", "/index/posts/uid/abc", nil))
	if rec.Code != 400 {
		t.Errorf("status=%d", rec.Code)
	}
}
...
func TestHandleSearchTags(t *testing.T) {
	...
	r := chi.NewRouter()
	r.Get("/index/tags/search", h.HandleSearchTags)
	rec := httptest.NewRecorder()
	r.ServeHTTP(rec, httptest.NewRequest("GET", "/index/tags/search?keyword=go&limit=5", nil))
	if rec.Code != 200 {
		t.Errorf("status=%d", rec.Code)
	}
}
func TestHandlePopularTags(t *testing.T) {
	h := &SyncHandler{tagIndexSvc: &mockTag{}, log: logger.NewNop()}
	r := chi.NewRouter()
	r.Get("/index/tags/popular", h.HandlePopularTags)
	rec := httptest.NewRecorder()
	r.ServeHTTP(rec, httptest.NewRequest("GET", "/index/tags/popular?limit=10", nil))
	if rec.Code != 200 {
		t.Errorf("status=%d", rec.Code)
	}
}
func TestHandleGetTag_Found(t *testing.T) {
	...
	h := &SyncHandler{tagIndexSvc: tg, log: logger.NewNop()}
	r := chi.NewRouter()
	r.Get("/index/tags/{tagUid}", h.HandleGetTag)
	rec := httptest.NewRecorder()
	r.ServeHTTP(rec, httptest.NewRequest("GET", "/index/tags/700", nil))
	if rec.Code != 200 {
		t.Errorf("status=%d", rec.Code)
	}
}
func TestHandleGetTag_NotFound(t *testing.T) {
	...
	r.Get("/index/tags/{tagUid}", h.HandleGetTag)
	rec := httptest.NewRecorder()
	r.ServeHTTP(rec, httptest.NewRequest("GET", "/index/tags/999", nil))
	if rec.Code != 404 {
		t.Errorf("status=%d", rec.Code)
	}
}
func TestHandleGetTagRegions(t *testing.T) {
	...
	h := &SyncHandler{tagIndexSvc: tg, log: logger.NewNop()}
	r := chi.NewRouter()
	r.Get("/index/tags/{tagUid}/regions", h.HandleGetTagRegions)
	rec := httptest.NewRecorder()
	r.ServeHTTP(rec, httptest.NewRequest("GET", "/index/tags/700/regions", nil))
	if rec.Code != 200 {
		t.Errorf("status=%d", rec.Code)
	}
}
