...
func (m *mockEvtLog) MarkProcessed(_ context.Context, ev *model.CrossRegionSyncEvent, _ string) error {
	m.m[ev.EventID] = true; return nil
}
...
func TestConsumerRouteEvent_PostCreated(t *testing.T) {
	...
	if err := c.routeEvent(context.Background(), ev); err != nil { t.Fatal(err) }
...
func TestConsumerRouteEvent_Unknown(t *testing.T) {
	...
	if err := c.routeEvent(context.Background(), &model.CrossRegionSyncEvent{EventType: "X"}); err != nil { t.Fatal(err) }
}

func TestConsumerRouteEvent_Error(t *testing.T) {
	c := &SyncConsumer{indexSvc: &mockIdx{insErr: errors.New("db")}, log: logger.NewNop()}
	ev := &model.CrossRegionSyncEvent{EventType: model.EventTypePostCreated, Payload: model.EventPayload{PostUid: 100}}
	if err := c.routeEvent(context.Background(), ev); err == nil { t.Fatal("expected err") }
}
...
func TestHandleMessage_Success(t *testing.T) {
	...
	if r != consumer.ConsumeSuccess { t.Errorf("result=%v", r) }
	if !el.m["e1"] { t.Error("not marked") }
}
...
func TestHandleMessage_InvalidJSON(t *testing.T) {
	...
	if r != consumer.ConsumeRetryLater { t.Errorf("result=%v", r) }
}
...
func TestHandleMessage_Idempotent(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if r != consumer.ConsumeSuccess { t.Error("dup should success") }
}
...
func TestHandleMessage_GDPRDenied(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if r != consumer.ConsumeSuccess { t.Error("deny still success") }
	if !el.m["gdpr"] { t.Error("marked") }
}
